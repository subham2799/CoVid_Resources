...
class Cylinder extends React.Component{
    ...
    componentDidMount(){
        fetch("https://sheetdb.io/api/v1/cdea2ba4h0tfe/?sheet=Cylinder")
        .then(res => res.json())
        .then(result => {
    ...
    render(){
        const allDataArray = this.state.resourceData.map(item => <ResourceView selected = {this.props.district} district = {item.District} name = {item.Name} contact = {item.Contact} verificationTime = {item["Verification Time"]} status = {item.Status}/>);
        let flag = false;
